...
protected:
    ...
    */
    ...
    Tout* get_input_parameter(const std::string &parameter_name, float fScale = 1.0)
    {
        try
        {
            return get_parameter<Tin, Tout>("parameters", parameter_name, fScale);
        }
        catch (std::domain_error &e)
        {
            std::cout << "[----------] get_input_parameter (" << parameter_name
    ...
    template <typename Tin, typename Tout>
    Tout* get_reference_parameter(const std::string &parameter_name, float fScale = 1.0)
    {
        try
        {
            return get_parameter<Tin, Tout>("references", parameter_name, fScale);
        }
        catch (std::domain_error &e)
        {
            std::cout << "[----------] get_reference_parameter (" << parameter_name
...
private:
    ...
    struct data_reader<T*> {
        ...
        {
            size_t buffer_size;
            return (T*) read_data_to_aligned_array(conf[test_type][index][type][parameter_name], buffer_size);
        }
    };

    template<typename Tin, typename Tout>
    struct data_reader_cvt {
        static Tout* read_parameter(const int index, const std::string &type,
                                 const std::string &parameter_name, float fScale = 1.0)
        {
            size_t buffer_size;
            auto p_data_0 = read_data_to_aligned_array(conf[test_type][index][type][parameter_name], buffer_size);
            auto p_data_in = (Tin *) p_data_0;
            auto p_data_out = (Tout *) p_data_0;
            for (size_t i = 0; i < buffer_size / sizeof(Tin); i ++) {
                p_data_out[i] = (Tout)(float(p_data_in[i]) / fScale);
            }
            return p_data_out;
        }
    };
    ...
    template <typename Tin, typename Tout>
    Tout* get_parameter(const std::string &type, const std::string &parameter_name, float fScale = 1.0)
    {
        return data_reader_cvt<Tin, Tout>::read_parameter(GetParam(), type, parameter_name, fScale);
    }
...
template <typename Tin, typename Tout>
inline Tout* generate_random_int_numbers(const long size, const unsigned alignment, const Tin lo_range,
                               const Tin up_range, float fscale)
{
    std::uniform_int_distribution<Tin> random(lo_range, up_range);

    auto array1 = (Tin*) aligned_malloc<char>(size * sizeof(Tin), alignment);
    auto array2 = (Tout*) aligned_malloc<char>(size * sizeof(Tout), alignment);

    array1 =  generate_random_numbers<Tin, std::uniform_int_distribution<Tin>>(size, alignment, random);

    for(long i = 0; i < size; i++)
        array2[i] = (Tout)(array1[i]/fscale);
...
template<typename T> struct DataTypeCvt;
#ifdef _BBLIB_SPR_
template<> struct DataTypeCvt<float16> {
    using Float = float16;
};
#endif
template<> struct DataTypeCvt<int16_t> {
    using Float = float;
};

enum class FP16_INT16 {
    FP16 = 0,
    INT16 = 1
};
...
template<typename Tin, typename Tout>
struct data_cvt {
    static void convert(const size_t len, Tin * pIn, Tout * pOut, float fScale = 1.0) {
        for (size_t i = 0; i < len; i ++) {
            pOut[i] = (Tout)(float(pIn[i]) / fScale);
        }
    }
};
